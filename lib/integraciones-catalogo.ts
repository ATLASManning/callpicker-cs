...
export const PROTOCOLO_SIN_INTEGRACION = {
  ...
  /**
   * Frase de cierre obligatoria, textual.
   *
   * Pedirla como regla en prosa no funcionó: Atlas daba las dos condiciones y
   * ofrecía la prueba, pero omitía a quién se canaliza — que es justo la parte
   * operativa. Se entrega como cadena a copiar, igual que se resolvió el
   * "(a través de Zapier)" de Pipedrive.
   */
...
export function seccionIntegraciones(): string {
  ...
  return (
    ...
    `  PERO TAMPOCO la descartes ni digas "no se puede". ${PROTOCOLO_SIN_INTEGRACION.titulo}. La respuesta correcta es: si se cumplen las dos condiciones —${PROTOCOLO_SIN_INTEGRACION.condiciones.join(' Y ')}— ${PROTOCOLO_SIN_INTEGRACION.siSeCumplen.toLowerCase()} Preguntar por esas dos condiciones es parte de la respuesta, no un extra. ${PROTOCOLO_SIN_INTEGRACION.limite}\n` +
    `  CIERRE OBLIGATORIO: toda respuesta sobre una plataforma que no está en el catálogo termina con esta frase, TEXTUAL y completa — es la parte operativa y sin ella el asesor no sabe qué hacer con el caso: "${PROTOCOLO_SIN_INTEGRACION.canalizacion}"\n` +
