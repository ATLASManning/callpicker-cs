 ...
 *
 * Sobre el layout: el documento es largo y se lee de corrido, así que ocupa
 * el ancho completo de la ventana en vez de una columna angosta. Las siete
 * responsabilidades van en columnas tipo mampostería —cada tarjeta mide
 * distinto y así se empaquetan sin dejar huecos— y las tres fases del
 * protocolo de baja van lado a lado, que es como se leen: una secuencia,
 * no una pila.
 */
export default function PerfilRolPage() {
  return (
    <div className="p-4 md:p-6 space-y-5">
      ...
      {/* ── Objetivo + Postura, lado a lado ─────────────────────────────── */}
      <div className="grid gap-4 xl:grid-cols-12 items-start">
        <div className="cp-card space-y-3 xl:col-span-5">
          ...
          <div className="rounded-lg p-3" style={{ background: 'rgba(0,87,255,0.08)', border: '1px solid rgba(0,87,255,0.25)' }}>
            ...
            <p className="text-sm text-textHi leading-relaxed">{OBJETIVO_PUESTO.metricaAncla}</p>
          </div>
        ...
        <div className="cp-card space-y-3 xl:col-span-7">
          <h3 className="text-xs font-semibold text-textMid uppercase tracking-wide">
            Postura requerida — los tres desplazamientos
          </h3>
          <div className="grid gap-3 sm:grid-cols-3">
            {POSTURA.map((p, i) => (
              <div key={i} className="rounded-lg p-3.5" style={{ background: 'rgba(148,163,184,0.08)' }}>
      ...
      {/* ── Responsabilidades, en mampostería ───────────────────────────── */}
      <div className="space-y-3">
        ...
        <div className="columns-1 lg:columns-2 2xl:columns-3 gap-4">
          {RESPONSABILIDADES.map(r => (
            <div key={r.clave} className="cp-card space-y-2.5 mb-4 break-inside-avoid">
      ...
      <div className="cp-card space-y-4" style={{ borderColor: 'rgba(248,113,113,0.4)' }}>
        ...
        {/* Las tres fases, en secuencia horizontal */}
        <div className="grid gap-3 lg:grid-cols-3 items-start">
          {PROTOCOLO_BAJA.map(f => {
            const col = COLOR_FASE[f.fase] ?? '#94A3B8'
            return (
              <div key={f.fase} className="rounded-lg p-3.5 h-full" style={{ background: `${col}12`, border: `1px solid ${col}40` }}>
        ...
        <div className="grid gap-4 lg:grid-cols-2 items-start pt-1">
          <div>
            <p className="text-[10px] uppercase tracking-wide text-textLow mb-2">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1 2xl:grid-cols-2">
              {RESPUESTAS_NO_ACEPTADAS.map((r, i) => (
                <div key={i} className="rounded-md px-2.5 py-2" style={{ background: 'rgba(248,113,113,0.06)' }}>
          ...
          <div>
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1 2xl:grid-cols-2">
              {EVALUACION_PERDIDA.map((e, i) => (
                <div key={i} className="rounded-md px-2.5 py-2" style={{ background: 'rgba(148,163,184,0.07)' }}>
      ...
      {/* ── Cadencia + Indicadores ──────────────────────────────────────── */}
      <div className="grid gap-4 xl:grid-cols-12 items-start">
        <div className="cp-card space-y-2.5 xl:col-span-4">
        ...
        <div className="cp-card space-y-3 xl:col-span-8">
          <h3 className="text-xs font-semibold text-textMid uppercase tracking-wide">
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="text-left text-[10px] uppercase text-textLow">
                  <th className="pb-2 pr-3 w-52">Indicador</th>
                  <th className="pb-2 pr-3">Definición</th>
                  <th className="pb-2 w-56">Meta</th>
                </tr>
              </thead>
              <tbody>
                {INDICADORES_ANCLA.map((ind, i) => (
                  <tr key={i} className="border-t border-border/40">
                    <td className="py-2 pr-3 font-medium text-textHi align-top">{ind.nombre}</td>
                    <td className="py-2 pr-3 text-textMid align-top leading-relaxed">{ind.definicion}</td>
                    <td className="py-2 text-textLow align-top leading-relaxed">{ind.meta}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="grid gap-4 xl:grid-cols-12 items-start">
        <div className="cp-card space-y-2 xl:col-span-7">
        ...
        <div className="cp-card space-y-2 xl:col-span-5">
          <h3 className="text-xs font-semibold text-textMid uppercase tracking-wide">Anti-perfil</h3>
          <ul className="grid gap-x-6 gap-y-1.5 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
            {ANTI_PERFIL.map((a, i) => (
              <li key={i} className="text-xs text-textMid leading-relaxed flex gap-2">
