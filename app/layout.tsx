...
const FORCE_LIGHT = `
  ...
  html, body { background: #DBEAFE !important; color: #0F172A !important; color-scheme: only light !important; }
  main { background: #DBEAFE !important; }
  ...
  return (
    <html lang="es" suppressHydrationWarning style={{ background: '#DBEAFE', colorScheme: 'only light' }}>
      ...
      <body className="flex h-screen overflow-hidden" style={{ background: '#EFF6FF', color: '#0F172A' }}>
        <Sidebar />
        <main className="flex-1 overflow-y-auto" style={{ background: '#DBEAFE' }}>
          {children}
        </main>
      </body>
    </html>
